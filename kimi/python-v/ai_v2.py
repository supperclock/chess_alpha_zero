...
def score_move_board(from_pos, to_pos, board_state, history, side):
    ...
    # 新增逻辑：检查移动后的棋子是否会被对方直接吃掉
        # 新增逻辑：检查移动后的棋子是否会被对方直接吃掉（考虑保护与交换）
    penalty = 0
    for y in range(ROWS):
        for x in range(COLS):
            piece = tmp_board[y][x]
            if piece and piece['side'] == opponent:
                if can_move_on(tmp_board, {'x': x, 'y': y}, to_pos):
                    ...
                    if not is_in_check_board(sim_board, opponent):
                        ...
                        if can_recapture:
                            # 交换判断：我方损失 victim，对方损失 attacker
    ...
    total_score = eval_score + check_bonus + base - penalty  # 减去被吃掉的棋子价值
    return total_score
